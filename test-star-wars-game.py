...
EXHAUST_PORT_WIDTH_M = TRENCH_WIDTH_M / 3
...
def is_close_to_launch_position():
    return math.abs( get_distance_to_launch_position() ) < 100
...
def render_barrier( canvas, barrier ):
    ...
    hw = w / 2				# Block Half Width
    hh = h / 2				# Block Half Height
...
def check_for_collisions():
    ...
    if current_barrier_index < len( barriers ):
        ...
        if pos[ 2 ] > barrier[ 0 ] and pos[ 2 ] < barrier[ 0 ] + barrier[ 1 ]:
            ...
            x1 = pos[ 0 ] - SHIP_WIDTH_M / 2
            x2 = x1 + SHIP_WIDTH_M
            y1 = pos[ 1 ] - SHIP_HEIGHT_M / 2
            y2 = y1 + SHIP_HEIGHT_M

            # Calculate block size
            bw = TRENCH_WIDTH_M / 3
            bh = TRENCH_HEIGHT_M / 3
            bhw = bw / 2
            bhh = bh / 2
            for by in range( -1, 2 ):
                by1 = by * bh - bhh
